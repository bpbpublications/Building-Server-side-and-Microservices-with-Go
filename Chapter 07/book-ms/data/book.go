...
func createBook(ctx context.Context, bookName, authorName, publisher string, description databasemodule.NullString) (response *BookEntity, err error) {
	...
	if rr.ScanNext() {
		...
		response.Description = databasemodule.GetNullStringValue(description)
...
func updateBook(ctx context.Context, bookID, bookName, authorName, publisher string, description databasemodule.NullString) (response time.Time, err error) {
	query := `
		update book
		set
			book_name = $1,
...
func changeBookStatus(ctx context.Context, bookID string, status int, userID databasemodule.NullString) (err error) {
	dbRunner := ctx.Value(values.ContextKeyDbRunner).(databasemodule.Runner)
